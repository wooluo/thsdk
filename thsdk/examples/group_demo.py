...
with THS() as ths:
    ...
    response = ths.block_components("URFI883917")
    df = pd.DataFrame(response.get_result())
    codes_list = df["代码"].tolist()

    if not target_group_id == "":
        response = ths.group_code_add(cookie, id=target_group_id, ths_codes=codes_list, version=version)
        if not response.is_success():
